...
export default function HospitalDashboardPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data } = await api.get('/api/hospitals/stats');
  ...
  const profileStatus = profile?.status as keyof typeof statusConfig | undefined;
  const status = profileStatus ? statusConfig[profileStatus] : statusConfig.pending;
      ...
      <div className="min-h-screen bg-[var(--color-bg-secondary)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="card card-hover p-6 mb-8 fade-in">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-[var(--color-text-primary)]">{profile?.hospitalName as string}</h1>
          ...
          <div className="space-y-6">
            ...
            {jobs.length > 0 ? (
              jobs.map((job) => {
                const applicants = (job?.applicants as unknown[]) || [];
                return (
                  <div key={job._id as string} className="card card-hover overflow-hidden fade-in">
                    <div className="p-6 border-b border-[var(--color-border)]">
                      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div>
                          <h3 className="text-xl font-bold text-[var(--color-text-primary)] mb-1">{job.title as string}</h3>
                          <div className="flex flex-wrap items-center gap-3 text-sm text-[var(--color-text-secondary)]">
                            <span className="flex items-center gap-1">
                              <Award className="h-4 w-4" />
                              {job.specialization as string}
                            </span>
                            <span>•</span>
                            <span className="flex items-center gap-1">
                              <MapPin className="h-4 w-4" />
                        ...
                        <div className="flex items-center gap-3">
                          <div className="px-4 py-2 bg-[var(--color-primary)]/10 rounded-xl">
                            <span className="text-2xl font-bold text-[var(--color-primary)]">{applicants.length}</span>
                      ...
                      {applicants.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                          {applicants.map((applicant) => {
                            ...
                            const resume = a.resume as string | undefined;
                            return (
                              <div
                                key={a._id as string}
                              ...
                              >
                                <div className="flex items-center gap-3 mb-3">
                                  ...
                                  <div>
                                    <p className="font-semibold text-[var(--color-text-primary)]">{a.name as string}</p>
                                    <p className="text-xs text-[var(--color-text-muted)]">{a.specialization as string} • {a.experience as number} yrs</p>
                                  </div>
                                </div>

                                <div className="flex gap-2">
                                  <a
                                    href={`mailto:${a.email as string}`}
