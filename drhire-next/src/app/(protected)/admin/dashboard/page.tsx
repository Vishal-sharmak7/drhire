...
import { ShieldCheck, Users, Building2, Briefcase, CheckCircle2, LayoutDashboard, FileCheck } from 'lucide-react';
...
interface Job {
  ...
  title: string;
  location: string;
  hospitalId: { hospitalName: string };
  applicants: unknown[];
}
...
export default function AdminDashboardPage() {
  ...
  const [pendingHospitalsState, setPendingHospitalsState] = useState<Hospital[]>([]);
  ...
  useEffect(() => {
    const fetchAdminData = async () => {
      try {
        const [hospitalsRes, usersRes, jobsRes, pendingRes] = await Promise.all([
          api.get('/api/admin/hospitals'),
          api.get('/api/admin/users'),
          api.get('/api/admin/jobs'),
          api.get('/api/admin/pending-hospitals'),
        ]);
  ...
  const renderSidebarItem = (id: string, icon: React.ReactNode, label: string, badgeCount?: number) => (
    <button
