...
import { Building2, MapPin, Briefcase, DollarSign, CheckCircle, ArrowLeft, Award, Users, Calendar } from 'lucide-react';
...
export default function JobDetailsPage() {
  ...
  useEffect(() => {
    const fetchJob = async () => {
      try {
        const { data } = await api.get<Job>(`/api/jobs/${params.id}`);
        setJob(data);
      } catch (error) {
  ...
  const handleApply = async () => {
    ...
    setApplying(true);
    try {
      await api.post(`/api/applications/${params.id}`);
  ...
  const hasApplied = job?.applicants?.some(a => a === user?._id || (a as { _id?: string })?._id === user?._id);

  return (
    <div className="min-h-screen bg-[var(--color-bg-secondary)] pb-12">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Link href="/jobs" className="inline-flex items-center gap-2 text-[var(--color-text-secondary)] hover:text-[var(--color-primary)] transition-colors mb-6">
              ...
              <div className="space-y-6">
                ...
                <div className="card p-6">
                  ...
                  {user?.role === 'doctor' ? (
                    ...
                    <Link href="/login" className="btn btn-primary w-full block text-center">Log in to Apply</Link>
                  )}
                </div>
              </div>
            </div>
